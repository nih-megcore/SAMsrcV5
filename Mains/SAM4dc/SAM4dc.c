    ...
    char    **argv)
{
    ...
    if(Params.NoiseHP != -999. && Params.NoiseLP != -999.) {
        ...
        sprintf(fpath, "%s/%s,%-d-%-dHz/Noise.cov", InputSAMpath, ParmName, (int)Params.CovHP, (int)Params.CovLP);
    ...
    for(n=0; n<Params.NumMark; n++) {
        ...
        if(!strncmp(Params.DirName, "NULL", 4))
            sprintf(Name1, "%s/%s,%s,%s,", OutputSAMpath, Prefix, ParmName, Stats[n].Name);
        else
